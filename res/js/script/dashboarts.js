function getLineChartsOptions (exam) {
	var y1 = [];
	var y2 = [];
	var x = [];
	var tt = 0;
	for (var i = 0; i < exam.length; i++) {
		let value = parseInt(exam[i].value);
		y1[i] = value
		tt += value;
		y2[i] = tt;
		x[i] = exam[i].time;
...
function getPieChartsOptions (tpo) {
	return option = {
		tooltip: { trigger: 'item', formatter: "{a} <br/>{b}: {c} ({d}%)" },
		legend: { data:['直达','营销广告','搜索引擎'] },
	...
	return option = {
	    ...
	    tooltip: {
	        ...
	        formatter: function (params) {
	            return params.name + ' <br/> 考试次数: ' + params.value[2] + "次";
	        }
	    },
		legend: {
	        orient: 'vertical',

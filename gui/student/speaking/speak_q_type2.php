...
<section class="">
	...
	?>
	<!-- read html -->
	<section  class="wrapper-xl scrollable read-panel d-n">
		<div class="text-center m-t-xl row">
			<div class="col-lg-8 col-lg-offset-2 text-left">
				<p class="h4 md m-t-xl text-dark font-400" ><i class="fa fa-clock-o m-r-sm"></i>Reading time: <span class="time">00:50</span></p>
				<div class="panel panel-default panel-body text-dark l-h-2x">
					<div class="inner-content d-n">
						<p class="h4 text-center m-t font-400"><?=$question['article']['title']?></p>
						<p class="page-info m-t-md padder"><?=$question['article']['main']?></p>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="text-center row m-t-xl">
			<div class="col-lg-6 col-lg-offset-3 text-left">
				...
				<div class="wrapper inner-tip">
					...
					<div class="m-t text-center ex-speaking-tip1">Preparation Time: <span class="r1">30</span> Seconds</div>
		...
		<div class="text-center row m-t-xl">
			<div class="col-lg-6 col-lg-offset-3 text-left">
				...
				<div class="m-t-lg wrapper inner-content">
					<div class="m-t text-center ex-tip2 font-400">Prepare you response</div>
	...
	<div class="d-n audiolist">
		...
		<!-- read-tip -->
		<audio id="read-audio">
			<source src="/res/audio/tfmk/<?=$question['readaudio']?>" type="audio/mpeg">
		</audio>
...
var pre, read,listen,question,tip2,tip1, ri;
$(function (){
	pre = document.getElementById('pre-audio');
	read = document.getElementById('read-audio');
	...
	$('.btn-countinue').on('click', function (){
		...
		if(progress == "readcontent"){
			clearInterval(ri);
			$('.read-panel').addClass('d-n');
	...
	pre.onended = function (){
		...
		$('.read-panel').removeClass('d-n');
		$('.btn-countinue').data('progress', 'read');
		read.play();
	}
	
	read.onended = function (){
		$('.inner-content').removeClass('d-n');
		$('.btn-countinue').data('progress', 'readcontent');
		let time = 50;
		ri = setInterval(function(){
			$('.time').html(sec2Time (time));
			if (time <= 0){
				clearInterval(ri);
				$('.read-panel').addClass('d-n');
				$('.listen-panel').removeClass('d-n');
